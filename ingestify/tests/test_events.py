...
class TestIngestionJobEventEmission:
    ...
    def setup_method(self):
        ...
        class MockTestSource(Source):
            ...
            def find_datasets(self, dataset_type, data_spec_versions, dataset_collection_metadata, **kwargs):
                return self._find_datasets_mock(dataset_type, data_spec_versions, dataset_collection_metadata, **kwargs)
        
        self.mock_source = MockTestSource("test_source")
        
        data_spec_versions = DataSpecVersionCollection.from_dict({"default": {"v1"}})
        self.selector = Selector.build({"competition_id": 11}, data_spec_versions=data_spec_versions)
    ...
    def test_selector_skipped_event_emitted_when_up_to_date(self):
        ...
        mock_metadata = Mock()
        mock_metadata.last_modified = datetime.datetime(2023, 1, 2)  # Newer than selector
    ...
    def test_dataset_skipped_event_emitted_when_should_refetch_false(self):
        ...
        dataset_skipped_calls = [
            call for call in self.mock_store.dispatch.call_args_list
